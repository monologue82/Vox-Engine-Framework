console.log('Script started loading...');
        ...
        const SystemMonitor = {
            ...
            // Cache DOM elements (updated for new HTML structure)
            cacheDOM() {
                this.dom = {
                    ...
                    lmstudioDot: document.getElementById('lmstudio-dot'),
                    audioDot: document.getElementById('audio-dot'),
                    // Resource elements
            ...
            async checkServerStatus() {
                try {
                    ...
                    if (response.ok) {
                        ...
                        this.updateServerStatus('online', `Online v${data.version || '0.4.1'}`);
            ...
            async checkLMStudioStatus() {
                try {
                    ...
                    if (response.ok) {
                        const data = await response.json();
                        if (data.providers && data.providers.includes('lmstudio')) {
                            this.updateLMStudioStatus('online', 'Ready');
                        } else {
                            this.updateLMStudioStatus('warning', 'Unavailable');
                        }
                    } else {
                        throw new Error('LM Studio check failed');
                    }
                } catch (error) {
                    this.updateLMStudioStatus('offline', 'Not Available');
                    console.warn('LM Studio check failed:', error);
                }
            },
            ...
            // Update server status UI - adapted for new structure
            updateServerStatus(status, text) {
                this.status.server = status;
                if (this.dom.serverStatus) {
                    this.dom.serverStatus.textContent = text;
                }
                if (this.dom.serverDot) {
                    this.dom.serverDot.className = 'status-dot ' + status;
                }
            },
            
            // Update LM Studio status UI
            updateLMStudioStatus(status, text) {
                this.status.lmstudio = status;
                if (this.dom.lmstudioStatus) {
                    this.dom.lmstudioStatus.textContent = text;
                }
                if (this.dom.lmstudioDot) {
                    this.dom.lmstudioDot.className = 'status-dot ' + status;
                }
            },
            ...
            updateAudioStatus(status, text) {
                ...
                if (this.dom.audioDot) {
                    this.dom.audioDot.className = 'status-dot ' + status;
                }
            },
            ...
            updateWebSocketStatus(status) {
                ...
                const wsDot = document.getElementById('websocket-dot');
                const wsStatus = document.getElementById('websocket-status');
                if (wsDot) wsDot.className = 'status-dot ' + status;
                if (wsStatus) wsStatus.textContent = status === 'connected' ? 'Connected' : 'Disconnected';
            },
            
            // Update resource usage - adapted for new HTML structure
            updateResourceUsage(type, data) {
                if (type === 'memory' && this.dom.memoryBar) {
                    ...
                    if (this.dom.memoryUsed) this.dom.memoryUsed.textContent = `${data.used_gb || 0} GB`;
                    if (this.dom.memoryTotal) this.dom.memoryTotal.textContent = `${data.total_gb || 0} GB`;
                    if (this.dom.memoryPercent) this.dom.memoryPercent.textContent = `${percent}%`;
                }
                
                if (type === 'cpu' && this.dom.cpuBar) {
                    this.resources.cpu = data;
                    this.dom.cpuBar.style.width = `${data}%`;
                    if (this.dom.cpuPercent) this.dom.cpuPercent.textContent = `${data}%`;
                }
                ...
                if (type === 'gpu' && data.available) {
                    ...
                    if (this.dom.vramBar) {
                        ...
                        if (this.dom.vramUsed) this.dom.vramUsed.textContent = `${data.vram_used_gb || 0} GB`;
                        if (this.dom.vramTotal) this.dom.vramTotal.textContent = `${data.vram_total_gb || 0} GB`;
                        if (this.dom.vramPercent) this.dom.vramPercent.textContent = `${vramPercent}%`;
                    }
                    ...
                    if (this.dom.gpuUtilBar) {
                        ...
                        if (this.dom.gpuUtil) this.dom.gpuUtil.textContent = `${gpuUtil}%`;
                    }
                }
            }
        };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Load and apply system monitor panel visibility
            loadSystemMonitorSetting();
            
            // Add error handlers to all vendor logos if they exist
        ...
        function loadSystemMonitorSetting() {
            fetch('/api/settings')
                .then(resp => resp.json())
                .then(settings => {
                    const enableMonitor = settings.system?.enable_monitor || false;
                    const panel = document.getElementById('system-monitor-panel');
                    if (panel) {
                        panel.style.display = enableMonitor ? 'block' : 'none';
                    }
                })
                .catch(err => console.warn('Failed to load monitor setting:', err));
        }
        
        // Init nav theme icon on page load
        (function() {
        ...
        let currentPrompt = '';
        let currentTranslationSeq = 0;  // 翻译序列号，用于过滤乱序结果
        ...
        function handleVoiceUploadFromInput(event) {
            const file = event.target.files[0];
            handleVoiceUpload(file);
        }
        window.handleVoiceUploadFromInput = handleVoiceUploadFromInput;
        ...
        // Initialize theme selector when DOM is loaded
                ...
                function loadAllOptions() {
                    ...
                    // Load browser microphone list
                    console.log('Loading browser microphones...');
                    loadBrowserMicrophones();

                    // Load streaming TTS setting
                    const streamingCheckbox = document.getElementById('streaming-tts');
                    if (streamingCheckbox) {
                        streamingCheckbox.checked = localStorage.getItem('streamingTTS') === 'true';
                    }

                    // Load translation models from LM Studio (single source)
                    function loadTranslationModels() {
                        const select = document.getElementById('model-select');
                        const providerSelect = document.getElementById('translation-provider');
                        if (!select) return;
                        
                        const provider = providerSelect ? providerSelect.value : 'lps';
                        ...
                        if (provider === 'lps') {
                            fetch('/api/providers')
                                ...
                                .then(providers => {
                                    select.innerHTML = '';
                                    if (providers.lps && providers.lps.models && providers.lps.models.length > 0) {
                                        providers.lps.models.forEach(m => {
                                            const option = document.createElement('option');
                                            option.value = m.model_path;
                                            option.textContent = m.vendor + '/' + m.model_name + ' (' + m.size_mb + 'MB)';
                                            select.appendChild(option);
                                        });
                                        // Set default from API response
                                        const defaultModel = providers.lps.default_model || '';
                                ...
                                .catch(error => {
                                    console.error('Failed to load LPS models:', error);
            ...
            switch(data.status) {
                case 'listening':
                    ...
                    addLogMessage('info', 'Listening - Speech recognition started');
                ...
                case 'translating':
                    ...
                    addLogMessage('info', 'Translating - Translation in progress');
                    break;
                case 'stopped':
                    statusEl.classList.add('status-idle');
                    addLogMessage('info', 'Recognition stopped');
                    break;
                default:
                    statusEl.classList.add('status-idle');
            }
        ...
        socket.on('recognition_result', (data) => {
            ...
            const recentTexts = lastRecognizedTexts.slice(-3);
            ...
            // ── Trigger Translation with Hy-MT2 style ──
            const targetLang = document.getElementById('target-language').value;
            const modelName = document.getElementById('model-select').value;
            
            socket.emit('translate_debug_text', {
                text: currentRecognition.trim(),
                model_name: modelName,
                source_lang: document.getElementById('source-language').value,
                target_lang: targetLang,
                translation_style: 'hy_mt2'
            });
        });
        ...
        socket.on('recognition_complete', (data) => {
            ...
            addLogMessage('success', `Recognition Complete - ${data.segments} segments in ${data.total_time}`);
        });
        ...
        function updateTranslationDisplay(data) {
            // 忽略过期的翻译结果（使用 !=null 避免 seq=0 的falsy问题）
            if (data.seq != null && data.seq < currentTranslationSeq) {
                return;
            }
            
            if (translationContentEl) {
                // 当翻译序列号变化时，重置流式TTS的处理进度
                if (data.seq != null && data.seq !== streamingTranslationSeq) {
                    streamingTranslationSeq = data.seq;
                    lastProcessedLength = 0;
                }
                ...
                if (data.char_count === data.chunk.length) {
                    addLogMessage('info', 'Translation is streaming...');
                }
        ...
        socket.on('translation_chunk', (data) => {
            ...
            currentTranslationSeq = data.seq != null ? data.seq : currentTranslationSeq;
            
            // 始终存储最新数据到 pending，避免油门窗口内的数据丢失
            ...
            if (now - lastTranslationUpdate >= TRANSLATION_UPDATE_INTERVAL) {
                ...
                translationRAF = requestAnimationFrame(() => {
                    translationRAF = null;
                    if (pendingTranslationUpdate) {
                        const latest = pendingTranslationUpdate;
                        pendingTranslationUpdate = null;
                        updateTranslationDisplay(latest);
                    }
                });
            }
        });
        ...
        socket.on('translation_complete', (data) => {
                ...
                if (selectedVoiceSample) {
                    ...
                    loadVoiceSamples().then(() => {
                        if (selectedVoiceSample) {
                            ...
                            addLogMessage('warning', 'No Voice Sample - Please upload and select a voice sample to enable auto-play');
                        }
                    });
                }
            }
            ...
            addLogMessage('success', 'Translation Complete - ' + message);
        });
        ...
        // ── Browser-based Audio Capture & Streaming ──
        ...
        // Start recognition: capture browser mic and stream to FunASR
        function startRecognition() {
            const micDeviceId = document.getElementById('microphone-select').value;
            const sourceLang = document.getElementById('source-language').value;
            const modelName = document.getElementById('model-select').value;
            const targetLang = document.getElementById('target-language').value;

            if (!micDeviceId) {
                addLogMessage('error', 'Please select a microphone');
                return;
            }

            if (!modelName) {
                addLogMessage('error', 'Please select a translation model');
                return;
            }
            ...
            addLogMessage('info', 'Starting Recognition - Microphone streaming to FunASR...');
            ...
            // Start browser microphone capture
            startBrowserMicCapture(micDeviceId, sourceLang, targetLang);
        }

        async function startBrowserMicCapture(micDeviceId, sourceLang, targetLang) {
            try {
                // PyAudio device IDs (prefixed with 'pyai_') are server-side indices,
                // not browser device IDs. Use default device in that case.
                const isPyAudioId = micDeviceId && micDeviceId.startsWith('pyai_');
                const constraints = {
                    audio: (micDeviceId && !isPyAudioId) ? { deviceId: { exact: micDeviceId } } : true,
                    video: false
                };
                ...
                console.log('Browser mic capture started, sourceLang:', sourceLang, 'targetLang:', targetLang);

            } catch (err) {
                console.error('Error starting browser mic capture:', err);
                addLogMessage('error', 'Microphone Error: Could not access microphone - ' + err.message);
                isProcessing = false;
                isStreaming = false;
                // Reset mic button
                ...
                if (btn) {
                    ...
                    document.getElementById('mic-status').className = 'status-indicator idle';
                }
            }
        }
        ...
        function stopRecognition() {
            ...
            addLogMessage('info', 'Recognition Stopped - Speech recognition has been stopped');
        }

        // Populate browser microphones
        async function loadBrowserMicrophones() {
            const select = document.getElementById('microphone-select');
            if (!select) return;

            select.innerHTML = '<option value="">Loading...</option>';

            // Try browser enumerateDevices with prior permission request
            if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
                try {
                    // Request mic permission briefly to get full device labels/IDs
                    let permStream = null;
                    try {
                        permStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
                    } catch (permErr) {
                        console.warn('Mic permission not granted for enumeration:', permErr.message);
                    }

                    // Enumerate devices (now with full labels if permission granted)
                    const devices = await navigator.mediaDevices.enumerateDevices();
                    ...
                    if (audioInputs.length > 0) {
                        ...
                        console.log('Loaded', audioInputs.length, 'browser microphone(s)');
                        return;
                    }
                } catch (err) {
                    console.warn('Browser enumerateDevices failed:', err.message);
                }
            }

            // Fallback: try server-side microphone API
            try {
                const response = await fetch('/api/microphones');
                if (response.ok) {
                    ...
                    if (mics.length > 0) {
                        ...
                        mics.forEach((mic, index) => {
                            const option = document.createElement('option');
                            option.value = 'pyai_' + (mic.index ?? index);
                            option.textContent = mic.name || `Microphone ${index + 1}`;
                            if (index === 0) option.selected = true;
                            select.appendChild(option);
                        });
                        console.log('Loaded', mics.length, 'server microphone(s) via PyAudio');
                        return;
                    }
                }
            } catch (err) {
                console.warn('Server microphone API failed:', err.message);
            }
        ...
        window.selectVoiceSample = selectVoiceSample;
